...
/*无展开*/
static void bdd_printset_rec_chenbaijun_single(FILE *ofile, int r, int *set)
{
	...
	if(r == 1)
	{
		...
		int maxWeight = getMaxWeightNaive(temp);
		if (solutionSpaceNum < top_k)
		{
			solutionSpace[solutionSpaceNum].weight = maxWeight;
			solutionSpace[solutionSpaceNum].str = temp;
		}
		/*达到topk建立小根堆*/
		if (solutionSpaceNum == top_k - 1)
		{
	...
	else
	{
		...
		bdd_printset_rec_chenbaijun_single(ofile, LOW(r), set);

		set[var] = 2;/*1*/
	
		bdd_printset_rec_chenbaijun_single(ofile, HIGH(r), set);
...
static void bdd_printset_rec_chenbaijun_singleOpen(int i,FILE *ofile, int r, int *set)
{
	if (r == 0)
	{
		return;
	...
	else
	{
		...
		if(var>i)
		{
			...
			bdd_printset_rec_chenbaijun_singleOpen(i+1,ofile, r, set);
		...
		else
		{
			...
			bdd_printset_rec_chenbaijun_singleOpen(i+1,ofile, LOW(r), set);
	
			set[var] = 2;/*1*/
	
			bdd_printset_rec_chenbaijun_singleOpen(i+1,ofile, HIGH(r), set);
...
static void bdd_printset_rec_chenbaijun(FILE *ofile, int r, int *set)
{
	...
	/*bdd_printset_rec_chenbaijun_single(ofile, r, set);*/
	bdd_printset_rec_chenbaijun_singleOpen(0,ofile, r, set);
	maxHeapSort(solutionSpace, top_k);
}



void initsolutionSpace(int k)
{
	top_k = k;
...
void bdd_printset_topk(BDD r, int topN)
{
	...
	bdd_printset_rec_chenbaijun(stdout, r, set);
	
	/* 打印topk 结果*/
	/*
	int index = 0;
	for (index = 0; index <(solutionSpaceNum<top_k ? solutionSpaceNum : top_k); index++)
	{
		printf("%s Weight:%d\n", solutionSpace[index].str, solutionSpace[index].weight);
	}
	
	printf("solutionSpaceNum==%d\n", solutionSpaceNum);
	*/
...
int* bdd_fprintset_topk(FILE *ofile, BDD r, int top_k)
{
	...
	int j = 0;
	int num = solutionSpaceNum<top_k ? solutionSpaceNum : top_k;
