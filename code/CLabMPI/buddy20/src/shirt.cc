...
bdd loadSubBDD(int index)
{
	...
	FILE *stream;
	sprintf(subBDDName, "%s%d%s", "/home/fengtao/chenbaijun/chenbaijun/bdd_", index, ".txt");
...
void *getTopK(void *ptr)
{
	...
	bdd_printset_topk(subBDD, top_k, initWeight, position, set, space, &solutionsetNum,&pruningWeight);
	...
	MPI_Send(&pruningWeight, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
	pthread_exit(0); 
}
void *communication(void *ptr)
{
	int myid=*(int *)ptr;
	//int message[3]={fromID,taskid,pruningWeight};
	pthread_t id;
	while(true)
	{
		//sleep(2);
		...
		if(minHeapVal>bestBcastWeight)
		{
			...
			setBound(bestBcastWeight);//726145
		}
...
main(int argc, char *argv[])  {
	...
	/*读取权重信息*/
	loadWeightProfile("/home/fengtao/chenbaijun/chenbaijun/shirt_profile.txt");
	...
	*/
	...
	if(myid == 0)
	{
		...
		for(int i=numprocs-1;i<partitionNum;i++)
		{
			...
			int pruningWeight=0;
			MPI_Recv(&pruningWeight, byteNum, MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
		...
		for (int i = 0; i < numprocs-1;i++)
		{
			...
			//int message[3]={fromID,taskid,pruningWeight};
			int message[3]={-1,-1,pruningWeight};
...
*/
	...
	*/
	...
	if(myid==0)
	{
		...
		cout<<"master sort:"<< end-start<<endl;
		for (int i = 0; i < top_k; i++)
		{
