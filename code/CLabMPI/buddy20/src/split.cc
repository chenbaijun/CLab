...
void bdd_partition(int partitionVarNum, bdd root)
{
	...
	if (partitionVarNum == 0)
	{
		...
		sprintf(Left_bdd_name, "/home/fengtao/chenbaijun/chenbaijun/bdd_%d.txt", partitionNumber);
		bdd_fnsave(Left_bdd_name, Left_bdd);
		partitionNumber++;
		
		
		char Right_bdd_name[50];
		sprintf(Right_bdd_name, "/home/fengtao/chenbaijun/chenbaijun/bdd_%d.txt", partitionNumber);
...
main(int argc, char *argv[])  {
	...
	init();
	bdd root = loadTree("/home/fengtao/chenbaijun/chenbaijun/bdd.txt");
